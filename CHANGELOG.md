...
- 9bfa496: **Fixes a release-pipeline bug that left `npm install -g @clawops/cli` on the previous
  version.**

  v1.7.8 published under the `legacy` dist-tag, as the 1.x line is meant to. But `latest` was
  _also_ a 1.x version at the time, so nothing moved it: the registry served `legacy: 1.7.8`
...
- f726684: **`clawops mcp wire` never wired anything, and the MCP HTTP server had no authentication.**
