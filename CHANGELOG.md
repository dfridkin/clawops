...
- b007d05: **Every MCP tool now sends its description.** The descriptions in `spec/mcp-tools.yaml` — each
...
- f2e0a04: The MCP registry manifest (`server.json`) is bumped when the version is, rather than rewritten in
  ...
  The registry itself was current at `2.0.2`; only the file in the repository was stale. (An
  earlier stall at `1.2.1`, which the release workflow's comments still describe, was fixed by
  WO-64 before this release.)
