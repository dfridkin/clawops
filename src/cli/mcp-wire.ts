...
/**
 * Backported from clawops 2.0 (WO-61). Commands run through `session.exec`, not the
 * privilege-escalating wrapper the 2.x line uses: that wrapper does not exist on this
 * branch, and every other `docker exec` here runs the same way. On AWS, where clawops
 * connects as `ubuntu` and only `clawops` is in the docker group, these calls fail — as the
 * rest of this line's day-two commands already do. The difference is that a failure is now
 * reported instead of being reported as success.
 */
 ...
 * verified against the config schemas of 2026.4.5, 2026.7.1-2 and 2026.9.2. Nothing on this
 * line validated the write, so clawops stored a key nothing read, restarted the gateway, and
 * reported success. Nothing was ever wired.
 */
...
async function supportsMcpAdd(session: SshSession, signal: AbortSignal): Promise<boolean> {
  const help = await session.exec(`${OC} add --help`, signal)
  return help.code === 0
}
...
export async function wireGatewayMcp(
  ...
  const existing = await session.exec(`${OC} show ${GATEWAY_MCP_NAME}`, signal)
  ...
  if (alreadyWired) {
    ...
    const removed = await session.exec(`${OC} unset ${GATEWAY_MCP_NAME}`, signal)
    if (removed.code !== 0) {
      return {
        status: 'probe-failed',
  ...
  const added = await session.exec(`${OC} add ${GATEWAY_MCP_NAME} ${flags}`, signal)
  if (added.code !== 0) {
  ...
  await session.exec(`${OC} reload`, signal)
