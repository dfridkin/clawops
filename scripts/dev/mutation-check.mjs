...
const MUTATIONS = [
  ...
  { name: 'safety docs miscount the destructive tools',
    file: 'docs/security/mcp-safety.md', from: 'the 8 destructive ones', to: 'the 7 destructive ones', test: 'tests/mcp/catalog.test.ts' },
  ...
  { name: 'the signal never reaches the connection attempt',
    file: 'src/transport/wait.ts', from: 'await connect({ ...conn, awaitingBoot: true, signal: opts.signal })', to: 'await connect({ ...conn, awaitingBoot: true })', test: 'tests/transport/wait.test.ts' },
  ...
  { name: 'a host-key mismatch is reported in ssh2 jargon again',
    file: 'src/transport/ssh.ts', from: '  if (/host.*(denied|verification)/i.test(message)) {', to: '  if (false) {', test: 'tests/transport/known-hosts.test.ts' },
  ...
  { name: 'a rewritten error stops being recognised as retryable',
    file: 'src/transport/ssh.ts', from: '\n  return { summary: `SSH connection failed: ${message}` }', to: '\n  return { summary: `SSH failed: ${message}` }', test: 'tests/transport/known-hosts.test.ts' },
  ...
  { name: 'apply closes the public ports without checking the tailnet answers',
    file: 'src/plan/apply.ts', from: '  await guardPrivateOnlyApply(plan, ctx, opts?.probeTailnet ?? ((conn) => probeSsh(conn, opts?.signal)))\n', to: '', test: 'tests/plan/apply.test.ts' },
  { name: 'a private-only plan that still opens a port is applied',
    file: 'src/plan/private-only.ts', from: '  if (network.allowedSshCidrs.length > 0 || network.allowedGatewayCidrs.length > 0) {', to: '  if (false) {', test: 'tests/plan/private-only.test.ts' },
  { name: 'a private-only plan made for an old tailnet address is applied',
    file: 'src/plan/private-only.ts', from: '  if (network.tailscale.ip !== current.ip) {', to: '  if (false) {', test: 'tests/plan/private-only.test.ts' },
  { name: 'an unreachable tailnet does not stop private-only',
    file: 'src/plan/private-only.ts', from: '  if (!(await probe(conn))) {', to: '  if (false) {', test: 'tests/plan/private-only.test.ts' },
  { name: 'plan --private-only skips the reachability check',
    file: 'src/cli/commands/plan.ts', from: '      await assertTailnetReachable(ctx, (conn) => probeSsh(conn))\n', to: '', test: 'tests/cli/plan.test.ts' },
  { name: 'apply stops recording whether the public ports are closed',
    file: 'src/plan/apply.ts', from: '  recordPublicExposure(ctx.stackName, plan)\n', to: '', test: 'tests/plan/apply.test.ts' },
  { name: 'buildContext ignores ignoreTailnet',
    file: 'src/cli/context.ts', from: 'args.ignoreTailnet === true ? undefined : config', to: 'config', test: 'tests/cli/context.test.ts' },
  { name: 'revert leaves the tailnet over the tailnet',
    file: 'src/harden/flows.ts', from: 'buildContext({ stack: ctx.stackName, ignoreTailnet: true })', to: 'buildContext({ stack: ctx.stackName })', test: 'tests/cli/harden-revert.test.ts' },
  { name: 'revert proceeds without the public address answering',
    file: 'src/harden/flows.ts', from: '  if (!(await probeSsh(publicConn))) {', to: '  if (false) {', test: 'tests/cli/harden-revert.test.ts' },
  { name: 'revert drops the override even when the host did not leave',
    file: 'src/harden/flows.ts', from: '  if (!left.ok) {', to: '  if (false) {', test: 'tests/cli/harden-revert.test.ts' },
  { name: 'revert leaves the tailnet key pinned',
    file: 'src/harden/flows.ts', from: '  forgetHost(expandHome(config.ssh.knownHostsPath), override.ip, publicConn.port)\n', to: '', test: 'tests/cli/harden-revert.test.ts' },
  ...
  { name: 'the harden tool changes a live host without confirming (R19)',
    file: 'src/mcp/tools/cli/harden.ts', from: '  if (!input.dryRun && !input.yes) {', to: '  if (false) {', test: 'tests/mcp/harden.test.ts' },
  { name: 'the harden tool cuts over to the tailnet even when a module failed',
    file: 'src/mcp/tools/cli/harden.ts', from: 'if (input.tailscale && !input.dryRun && failed.length === 0) {', to: 'if (input.tailscale && !input.dryRun) {', test: 'tests/mcp/harden.test.ts' },
  { name: 'the harden tool accepts join and leave in one call',
    file: 'src/mcp/tools/cli/harden.ts', from: '  if (input.tailscale && input.tailscaleRevert) {', to: '  if (false) {', test: 'tests/mcp/harden.test.ts' },
  { name: 'the harden tool reports a failed module as success',
    file: 'src/mcp/tools/cli/harden.ts', from: '  return failed.length > 0 ? errText(content) : okText(content)', to: '  return okText(content)', test: 'tests/mcp/harden.test.ts' },
  { name: 'the plan tool ignores privateOnly',
  ...
  { name: 'a dropped packet and a refused connection get the same advice',
    file: 'src/transport/ssh.ts', from: "  if (message.includes('ECONNREFUSED')) {", to: '  if (/ECONNREFUSED|ETIMEDOUT|EHOSTUNREACH/.test(message)) {', test: 'tests/transport/known-hosts.test.ts' },
  { name: 'the firewall diagnosis stops recognising a dropped packet',
    file: 'src/transport/ssh.ts', from: '  if (/ETIMEDOUT|EHOSTUNREACH|ENETUNREACH|Timed out while waiting for handshake/.test(message)) {', to: '  if (/ETIMEDOUT/.test(message)) {', test: 'tests/transport/known-hosts.test.ts' },
  { name: 'ssh2 wording is replaced rather than kept, so isTransient stops matching',
    file: 'src/transport/ssh.ts', from: '        `is listening on that port (${message}).`,', to: '        `is listening on that port.`,', test: 'tests/transport/known-hosts.test.ts' },
  { name: 'the boot wait gets the day-2 diagnoses after all',
    file: 'src/transport/ssh.ts', from: '  if (opts.awaitingBoot) return { summary: `SSH connection failed: ${message}` }', to: '  if (false) return { summary: `SSH connection failed: ${message}` }', test: 'tests/transport/known-hosts.test.ts' },
  { name: 'waitForSsh stops telling connect it is waiting on a boot',
    file: 'src/transport/wait.ts', from: 'await connect({ ...conn, awaitingBoot: true, signal: opts.signal })', to: 'await connect({ ...conn, signal: opts.signal })', test: 'tests/transport/wait.test.ts' },
  { name: 'a host-key mismatch is suppressed during a boot wait, where waiting cannot fix it',
    file: 'src/transport/ssh.ts', from: '  if (/host.*(denied|verification)/i.test(message)) {', to: '  if (!opts.awaitingBoot && /host.*(denied|verification)/i.test(message)) {', test: 'tests/transport/known-hosts.test.ts' },
  { name: 'the key clawops offered is called wrong instead of possibly absent',
    file: 'src/transport/ssh.ts', from: "      remedy: key\n        ? `clawops offered the key at ${key}. Its public half may not be installed on the host — `", to: "      remedy: key\n        ? `clawops offered the key at ${key}. It is the wrong key — `", test: 'tests/transport/known-hosts.test.ts' },
  { name: 'doctor packs the whole diagnosis into the detail column again',
    file: 'src/diagnostics/index.ts', from: '  const [summary, ...rest] = message.split(\'\\n\')', to: '  const [summary, ...rest] = [message]', test: 'tests/diagnostics/doctor.test.ts' },
]
