...
const MUTATIONS = [
  ...
  { name: 'revert leaves the tailnet over the tailnet',
    file: 'src/cli/commands/harden.ts', from: 'buildContext({ stack: ctx.stackName, ignoreTailnet: true })', to: 'buildContext({ stack: ctx.stackName })', test: 'tests/cli/harden-revert.test.ts' },
  { name: 'revert proceeds without the public address answering',
    file: 'src/cli/commands/harden.ts', from: '  if (!(await probeSsh(publicConn))) {', to: '  if (false) {', test: 'tests/cli/harden-revert.test.ts' },
  { name: 'revert drops the override even when the host did not leave',
    file: 'src/cli/commands/harden.ts', from: '  if (!left.ok) {', to: '  if (false) {', test: 'tests/cli/harden-revert.test.ts' },
