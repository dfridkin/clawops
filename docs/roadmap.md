...
| Group | Items | Notes |
|---|---|---|
| **Carried from the 1.x waves** | WO-34 (Tailscale), partly | Azure hardening landed in WO-32. Tailscale joins the host to a tailnet; the steps that repoint clawops at the new address and remove public access are still open |
...
| **v1.8** | WO-31, WO-32, WO-34 | GCP and Azure hardening shipped. Tailscale joins the host; repointing and private-only remain |
...
| WO-34: Tailscale VPN integration | ◐ | Shipped: install, join, report the address, as an off-by-default hardening module. Not yet: rewriting clawops config to the Tailscale address, `--private-only`, revert, doctor check |
