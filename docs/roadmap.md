...
| Work order | Status | Deliverable |
|---|---|---|
| WO-28 — Gateway-agent MCP client config | ⚠️ | `clawops mcp wire` command; optional wizard step. **Shipped non-functional in v1.5** — wrote `gateway.mcpClients`, a key OpenClaw has never had. Fixed in v1.7.8 |
