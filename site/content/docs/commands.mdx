...
| `plan` | Generate a deploy plan without applying it (`--ssh-cidr`, `--gateway-cidr`, `--publish-gateway` say who may connect; `--instance-type` picks the machine size; `--private-only` closes public access on a stack reached over its tailnet) |
...
| Command | Does |
|---|---|
| `harden` | Apply hardening modules: ssh, ufw, fail2ban, unattended-upgrades, auditd. `--tailscale` moves the stack onto its tailnet address; `--tailscale-revert` moves it back |
