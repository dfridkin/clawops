---
title: Security
description: What clawops protects, what it deliberately does not, and what leaves your network.
---

clawops provisions cloud infrastructure and hands an agent the keys to it, so it is worth being
precise about the boundaries.
...
unless `yes: true` is passed deliberately.

Three modes, and most setups want the first:
...
it is not a substitute for the agent having no shell.
...
Not redacted, deliberately: resource ids, ARNs, regions, instance types, stack names, file paths.
An audit trail that hides these is not useful.
